...
const (
	...
	userAgent      = "screenshot-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultScreenshotAPIURL is the default Screenshot API URL.
const defaultScreenshotAPIURL = `https://website-screenshot.whoisxmlapi.com/api/v1`
...
type ClientParams struct {
	...
	// ScreenshotAPIBaseURL is the endpoint for 'Screenshot API' service
	ScreenshotAPIBaseURL *url.URL
}
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.ScreenshotAPIBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultScreenshotAPIURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.ScreenshotAPIService = &screenshotAPIServiceOp{client: client, baseURL: apiBaseURL}

	return client
}

// Client is the client for Screenshot API services.
type Client struct {
	client *http.Client
	...
	// ScreenshotAPI is an interface for Screenshot API
